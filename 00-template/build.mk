...
# -----------------------------------
# Build rules for creating memory cards
ifneq ($(MEMCARD),)
$(MEMCARD):
	dd if=/dev/zero of=$@ bs=2048 count=1 status=none
endif
...
clean:
	find . \( -name '*~' -or -name '*.mc' -or -name '*.o' -or -name '*.rel' -or -name '*.elf' -or -name '*.ihx' \) -not -path './build/assets/*' -delete
